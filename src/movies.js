...
function howManyMovies(moviesArray) {
  const steveMovies = moviesArray.filter(movie => {
...
function scoresAverage(moviesArray) {
    const scores = moviesArray.reduce((acc,currentVal)=>{
      return acc + currentVal.score / movies.length
    },0)
...
function dramaMoviesScore(moviesArray) {
  ...
  // const dramaScores = moviesArray.reduce((acc,currentVal)=>{
  //     return acc + currentVal.score / dramaMovies.length 
  //   })
  // },0)
  // // return Math.round(scores * 100)/100
...
function orderByYear(moviesArray) {
  const sorted1 = [...moviesArray].sort((a,b)=>{
    if (a.year > b.year){
      return 1
...
function orderAlphabetically(moviesArray) {
  const order1 = [...moviesArray].sort((a,b) => {
    if (a.title === b.title){
    return 0
  } else if(a.title > b.title){
    return 1
